...
function formatReportTimestamp(isoString = nowIso()) {
...
const REPORT_DEFINITIONS = [
  {
    ...
    "CAIXA Laurentino",
    "Conta Doca",
    "Viacredi Finan-Laurentino",
    "CEF Finan",
    "Conta Finan Viacredi",
    "Aplicação Viacredi Finan- Laurentino",
    "Aplicação CEF Finan",
    "CEF Florença",
    "Sicredi Florença",
    "Aplicação CEF Florença",
    "Viacredi DCBL",
    "Viacredi Otto",
    "CEF Otto Oestreich",
    "Aplicação Viacredi Otto Oestreich",
    "Aplicação CEF Otto",
    "Viacredi Villa das Nações",
    "Aplicação Viacredi Villa das Nações",
    "Viacredi Grupo Laurentino"
],
    documentos: ['CONTRATO'],
...
function getSelectionPersistenceConfig(modalDefinition) {
  switch (modalDefinition && modalDefinition.key) {
    case 'contasCorrente':
      return {
        inputName: 'contaCorrente',
        entityName: 'entity.contaCorrentePK',
      };
    default:
      return null;
  }
...
async function selectViaModal({
  ...
  const selectAllFilteredRows = false;
  ...
  try {
    ...
    try {
      ...
      if (mainFrameBefore && modalChecked && modalChecked.length) {
        await writeSelectedEntitiesIntoMainFrame(mainFrameBefore, selectedSoFar && selectedSoFar.length ? selectedSoFar : modalChecked, persistenceConfig).catch(() => {});
        logEvent({ level: 'info', message: 'Persisted accumulated modal selections into main frame (pre-click)', detail: JSON.stringify((selectedSoFar && selectedSoFar.length ? selectedSoFar : modalChecked).slice(0,10)), modalTitle });
      }
    } catch (e) {}
  } catch (_) {}
  ...
  try {
    ...
    try {
      const modalCheckedPersist = await modalFrame.evaluate(() => Array.from(document.querySelectorAll('tr')).filter(r => { try { const cb = r.querySelector('input[type="checkbox"]'); return !!(cb && cb.checked); } catch(e){return false;} }).map(r => r.innerText.trim()).slice(0,200)).catch(() => []);
      const finalSelections = selectedSoFar && selectedSoFar.length ? selectedSoFar : modalCheckedPersist;
      if (mainFrame2 && finalSelections && finalSelections.length) {
        await writeSelectedEntitiesIntoMainFrame(mainFrame2, finalSelections, persistenceConfig).catch(() => {});
        logEvent({ level: 'info', message: 'Wrote accumulated selections into main frame', detail: JSON.stringify(finalSelections.slice(0,10)), modalTitle });
      }
    } catch (e) {}
    if (mainFrame2) {
      try {
        ...
        const selectedLists = await mainFrame2.evaluate(() => {
          ...
          try {
            ...
            try {
              ...
              const expected = uniqueNonEmpty(selectedSoFar || []);
              const missing = expected.filter(s => !flat.includes(s.trim()));
              if (missing && missing.length) {
                await writeSelectedEntitiesIntoMainFrame(mainFrame2, expected, persistenceConfig).catch(() => {});
...
async function selectViaModalByGrid({
  ...
  const persistenceConfig = getSelectionPersistenceConfig(modalDefinition);
  const selectAllFilteredRows = false;
  ...
  for (const currentValue of entries) {
    ...
    while (attempt < 3 && !succeeded) {
      ...
      try {
        ...
        const quickMarked = !selectAllFilteredRows && await modalFrame.evaluate(({ needle, rowCheckboxSel }) => {
          try {
            const normalize = s => {
              try { return String(s || '').normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[^\w\s]/g, ' ').replace(/\s+/g, ' ').trim().toLowerCase(); } catch (e) { return String(s || '').replace(/[^\w\s]/g, ' ').replace(/\s+/g, ' ').trim().toLowerCase(); }
            };
            ...
            await modalFrame.waitForFunction((needle) => {
              try {
                ...
                return Array.from(document.querySelectorAll('tr')).some(row => {
                  ...
                  if (style.display === 'none' || style.visibility === 'hidden' || row.offsetParent === null) {
                    return false;
                  }
                  ...
                  return Array.from(row.querySelectorAll('td,th')).some(cell => normalize(cell.innerText || '').includes(needle));
                });
              } catch (e) {
                return false;
              }
        ...
        if (selectAllFilteredRows) {
          const bulkVisible = await modalFrame.evaluate(({ needle, rowCheckboxSel, rowValueSelectors }) => {
            try {
              const normalize = s => String(s || '').normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[^\w\s]/g, ' ').replace(/\s+/g, ' ').trim().toLowerCase();
              const selectedRows = [];
              const matchesRow = (row) => {
                if (!row) return false;
                const style = window.getComputedStyle(row);
                if (style.display === 'none' || style.visibility === 'hidden' || row.offsetParent === null) return false;
                const candidates = [row.innerText || ''];
                for (const selector of rowValueSelectors || []) {
                  try {
                    for (const cell of Array.from(row.querySelectorAll(selector))) {
                      candidates.push(cell.innerText || '');
                    }
                  } catch (e) {}
                }
                candidates.push(...Array.from(row.querySelectorAll('td,th')).map(cell => cell.innerText || ''));
                return candidates.some(text => normalize(text).includes(needle));
              };
              for (const row of Array.from(document.querySelectorAll('tr'))) {
                ...
                const style = window.getComputedStyle(row);
                const isVisible = style.display !== 'none' && style.visibility !== 'hidden' && row.offsetParent !== null;
                if (!isVisible) continue;
                if (!checkbox.checked) {
                  try { checkbox.click(); } catch (e) { try { checkbox.checked = true; } catch (_) {} }
                  try { checkbox.dispatchEvent(new Event('change', { bubbles: true })); } catch (_) {}
                }
                if (checkbox.checked) selectedRows.push(normalize(row.innerText || ''));
              }
              return { selectedRows };
            } catch (e) {
              return { selectedRows: [] };
            }
          }, { needle: normalizedToken, rowCheckboxSel: modalDefinition.rowCheckboxSelector, rowValueSelectors: modalDefinition.rowValueSelectors || [] }).catch(() => ({ selectedRows: [] }));

          if (bulkVisible && bulkVisible.selectedRows && bulkVisible.selectedRows.length) {
            for (const rowText of bulkVisible.selectedRows) {
              if (rowText && !selectedSoFar.includes(rowText)) selectedSoFar.push(rowText);
            }
            try {
              const iframeLocator2 = page.locator('iframe#iFramePage');
              const mainFrame2 = await iframeLocator2.contentFrame().catch(() => null);
              if (mainFrame2 && persistenceConfig) {
                await writeSelectedEntitiesIntoMainFrame(mainFrame2, selectedSoFar, persistenceConfig).catch(() => {});
              }
            } catch (_) {}
            logEvent({ level: 'info', message: `Conta corrente: bulk selecionou resultados filtrados para ${currentValue}`, modalTitle, detail: JSON.stringify({ count: bulkVisible.selectedRows.length, sample: bulkVisible.selectedRows.slice(0, 10) }) });
            succeeded = true;
            break;
          }
        }
        ...
        if (selectAllFilteredRows) {
          const modalCheckedNow = await modalFrame.evaluate(({ needle, rowCheckboxSel, rowValueSelectors }) => {
            try {
              const normalize = s => String(s || '').normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[^\w\s]/g, ' ').replace(/\s+/g, ' ').trim().toLowerCase();
              const selectedRows = [];
              const matchesRow = (row) => {
          ...
          }, { needle: normalizedToken, rowCheckboxSel: modalDefinition.rowCheckboxSelector, rowValueSelectors: modalDefinition.rowValueSelectors || [] }).catch(() => []);

          if (!modalCheckedNow.length) {
            await saveDebugState('bulk-no-results').catch(() => {});
            logEvent({ level: 'warn', message: `Bulk DOM não encontrou resultados para ${currentValue}`, modalTitle });
            if (attempt >= 3) break;
            await page.waitForTimeout(300 * attempt);
            continue;
          }

          for (const item of modalCheckedNow) {
            if (item && !selectedSoFar.includes(item)) selectedSoFar.push(item);
          }

          try {
            const iframeLocator2 = page.locator('iframe#iFramePage');
            const mainFrame2 = await iframeLocator2.contentFrame().catch(() => null);
            if (mainFrame2 && persistenceConfig && selectedSoFar.length) {
              await writeSelectedEntitiesIntoMainFrame(mainFrame2, selectedSoFar, persistenceConfig).catch(() => {});
            }
          } catch (_) {}

          logEvent({ level: 'info', message: `Conta corrente: bulk selecionou resultados filtrados para ${currentValue}`, modalTitle, detail: JSON.stringify({ count: modalCheckedNow.length, sample: modalCheckedNow.slice(0, 10) }) });
          succeeded = true;
          break;
        }
        ...
        try {
          ...
          try {
            const iframeLocator2 = page.locator('iframe#iFramePage');
            const mainFrame2 = await iframeLocator2.contentFrame().catch(() => null);
            if (mainFrame2 && persistenceConfig && selectedSoFar && selectedSoFar.length) {
              await writeSelectedEntitiesIntoMainFrame(mainFrame2, selectedSoFar, persistenceConfig).catch(() => {});
    ...
    if (finalModal && selectedSoFar && selectedSoFar.length) {
      try {
        ...
        try {
          ...
          if (mainFrame2 && persistenceConfig) {
            await writeSelectedEntitiesIntoMainFrame(mainFrame2, selectedSoFar, persistenceConfig).catch(() => {});
            saveShot(page, 'persistense-config');
          }
        } catch (_) {}
      } catch (_) {}
    }
...
async function generateSingleReport(
  ...
  try {
    ...
    const finalPdfPath = path.join(
      REPORT_OUTPUT_DIR,
      `${sanitizeFileName(report.pdfName)}-${reportTimestamp}.pdf`
    );
